...
def main():
    # config:{'normalize': True, 'image_size': 352, 'metric': 'metrics.FixedIntervalMetrics', 'batch_size': 1, 'test_dataset': 'pascal_zs', 'sigmoid': True, 'name': 'pas_t', 'mask': 'text', 'splits': [0], 'custom_threshold': 0.24}  train_checkpoint_id:'rd64-uni-phrasepas5i-0'
...
def score(config, train_checkpoint_id, train_config):
    ...
    # {'prompt': 'shuffle+', 'version': 'ViT-B/16', 'extract_layers': [3, 7, 9], 'reduce_dim': 64}
    ...
    if config.test_dataset == 'pascal':
        ...
        for split in splits:
            ...
            metric = FixedIntervalMetrics(resize_pred=True, **metric_args)  # metric_args：{'sigmoid': True, 'custom_threshold': 0.24}
