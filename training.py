...
def load_group_vit(device):
    pthfile = 'group_vit_gcc_redcap_30e-3dd09a76.pth'
    net = torch.load(pthfile, map_location=torch.device('cuda'))
    group_vit = GroupViT().to(device)
    group_vit_dict = list(group_vit.state_dict().keys())
    i = 0
    for i in range(len(group_vit_dict)):
        param_name = group_vit_dict[i]
        group_vit.load_state_dict({param_name: net['model'][f'img_encoder.{param_name}']}, strict=False)
    group_vit.eval()
    return group_vit


group_vit = load_group_vit('cpu')
        ...
        while True:
            for data_x, data_y in data_loader:
                ...
                if config.mix:
                    ...
                    # 先决定参数
                    dims = 512 # 所有头总共需要的输入维度
                    heads = 8    # 单注意力头的总共个数
                    dropout_pro = 0.0 # 单注意力头
                    # 传入参数得到我们需要的多注意力头
                    
                    from torchvision import transforms
                    img = transforms.CenterCrop((224,224))(data_x[2])

                    visual_s_cond = torch.nn.Linear(384, 512)(group_vit(img)).to(text_cond.device)
                    # print(visual_s_cond.shape)
                    cond = torch.stack([visual_s_cond, text_cond],dim=1).to(text_cond.device).type(torch.float16)

                    mha = torch.nn.MultiheadAttention(embed_dim = dims, num_heads = heads, dropout = dropout_pro,dtype=torch.float16).to(text_cond.device)
                    attn_output, attn_output_weights = mha(cond, cond, cond)

                    cond = attn_output.mean(dim=1).type(torch.float32)
